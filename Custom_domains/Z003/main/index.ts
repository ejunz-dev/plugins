...
export async function apply(ctx: Context) {
    ...
    ctx.once('handler/before/DomainEdit#get', async (that) => {
        if (that.domain._id === 'Z003') 
            if (that.domain.z003_announce1) {
            SettingModel.DomainSetting(
                SettingModel.Setting('setting_domain', 'z003_announce1', '', 'markdown', 'Z003 Announce1'),
            );
            if (!that.domain.z003_announce2) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z003_announce2', '', 'markdown', 'Z003 Announce2'),
                );
            }
            if (!that.domain.z003_announce3) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z003_announce3', '', 'markdown', 'Z003 Announce3'),
                );
            }
            if (!that.domain.z003_announce4) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z003_announce4', '', 'markdown', 'Z003 Announce4'),
                );
            }
            if (!that.domain.z003_announce5) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z003_announce5', '', 'markdown', 'Z003 Announce5'),
                );
            }
            if (!that.domain.z003_announce6) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z003_announce6', '', 'markdown', 'Z003 Announce6'),
                );
            }       
         }
    });
