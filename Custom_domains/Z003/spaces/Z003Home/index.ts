...
export class Z003HomeBaseHandler extends Handler {
...
export class Z003HomeHandler extends Z003HomeBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const z003homeConfig = this.domain.z003home_config;
        console.log('z003homeConfig', z003homeConfig);

        // 检查 processingConfig 是否为 undefined
        if (!z003homeConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 z003home' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('z003homeConfig', z003homeConfig);
        const info = yaml.load(z003homeConfig) as any;
        ...
        this.response.template = 'z003home_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('z003home_main', '/z003home', Z003HomeHandler, PERM.PERM_VIEW_Z003HOME);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('z003home')) {
            console.log('Z003Home Domain pass');
            return true;
        }
        console.log('Z003Home Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'z003home') {
                ...
                console.log('Z003Home SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('Z003Home SystemConfig pass');
                    return true;
                }else{
                    console.log('Z003Home SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'z003home_main', { prefix: 'z003home' }, CheckAll);

   ctx.on('handler/after/Home#get', async (that) => {
    
        if (that.domain._id === 'Z003') {
            that.response.redirect = that.url('z003home_main');
        }
