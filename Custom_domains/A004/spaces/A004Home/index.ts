...
export class A004HomeBaseHandler extends Handler {
...
export class A004HomeHandler extends A004HomeBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const a004homeConfig = this.domain.a004home_config;
        console.log('a004homeConfig', a004homeConfig);

        // 检查 processingConfig 是否为 undefined
        if (!a004homeConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 a004home' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('a004homeConfig', a004homeConfig);
        const info = yaml.load(a004homeConfig) as any;
        ...
        this.response.template = 'a004home_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('a004home_main', '/a004home', A004HomeHandler, PERM.PERM_VIEW_A004HOME);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('a004home')) {
            console.log('A004Home Domain pass');
            return true;
        }
        console.log('A004Home Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'a004home') {
                ...
                console.log('A004Home SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('A004Home SystemConfig pass');
                    return true;
                }else{
                    console.log('A004Home SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'a004home_main', { prefix: 'a004home' }, CheckAll);

   ctx.on('handler/after/Home#get', async (that) => {
    
        if (that.domain._id === 'A004') {
            that.response.redirect = that.url('a004home_main');
        }
