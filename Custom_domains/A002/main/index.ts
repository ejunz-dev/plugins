...
export async function apply(ctx: Context) {
    ctx.i18n.load('en', {
        'a002home_main': 'Valorant',
        'a002home': 'Valorant',
    });


    ctx.i18n.load('zh', {
        'a002home_main': '瓦洛兰特频道',
        'a002home': '瓦洛兰特频道',
    });
    ...
    ctx.once('handler/before/DomainEdit#get', async (that) => {
        if (that.domain._id === 'A002') 
            if (!that.domain.a002_announce1) {
            SettingModel.DomainSetting(
                SettingModel.Setting('setting_domain', 'a002_announce1', '', 'markdown', 'A002 Announce1'),
            );
            if (!that.domain.a002_announce2) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a002_announce2', '', 'markdown', 'A002 Announce2'),
                );
            }
            if (!that.domain.a002_announce3) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a002_announce3', '', 'markdown', 'A002 Announce3'),
                );
            }
            if (!that.domain.a002_announce4) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a002_announce4', '', 'markdown', 'A002 Announce4'),
                );
            }
            if (!that.domain.a002_announce5) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a002_announce5', '', 'markdown', 'A002 Announce5'),
                );
            }
            if (!that.domain.a002_announce6) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a002_announce6', '', 'markdown', 'A002 Announce6'),
                );
            }       
         }
    });
