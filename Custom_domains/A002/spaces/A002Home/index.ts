...
export class A002HomeBaseHandler extends Handler {
...
export class A002HomeHandler extends A002HomeBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const a002homeConfig = this.domain.a002home_config;
        console.log('a002homeConfig', a002homeConfig);

        // 检查 processingConfig 是否为 undefined
        if (!a002homeConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 a002home' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('a002homeConfig', a002homeConfig);
        const info = yaml.load(a002homeConfig) as any;
        ...
        this.response.template = 'a002home_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('a002home_main', '/a002home', A002HomeHandler, PERM.PERM_VIEW_A002HOME);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('a002home')) {
            console.log('A002Home Domain pass');
            return true;
        }
        console.log('A002Home Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'a002home') {
                ...
                console.log('A002Home SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('A002Home SystemConfig pass');
                    return true;
                }else{
                    console.log('A002Home SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'a002home_main', { prefix: 'a002home' }, CheckAll);

   ctx.on('handler/after/Home#get', async (that) => {
    
        if (that.domain._id === 'A002') {
            that.response.redirect = that.url('a002home_main');
        }
