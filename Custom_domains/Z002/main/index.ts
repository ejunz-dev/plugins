...
export async function apply(ctx: Context) {
    ...
    ctx.once('handler/before/DomainEdit#get', async (that) => {
        if (that.domain._id === 'Z002') 
            if (that.domain.z002_announce1) {
            SettingModel.DomainSetting(
                SettingModel.Setting('setting_domain', 'z002_announce1', '', 'markdown', 'Z002 Announce1'),
            );
            if (that.domain.z002_announce2) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z002_announce2', '', 'markdown', 'Z002 Announce2'),
                );
            }
            if (that.domain.z002_announce3) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z002_announce3', '', 'markdown', 'Z002 Announce3'),
                );
            }
            if (that.domain.z002_announce4) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z002_announce4', '', 'markdown', 'Z002 Announce4'),
                );
            }
            if (that.domain.z002_announce5) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z002_announce5', '', 'markdown', 'Z002 Announce5'),
                );
            }
            if (that.domain.z002_announce6) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z002_announce6', '', 'markdown', 'Z002 Announce6'),
                );
            }       
         }
    });
