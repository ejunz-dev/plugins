...
export class Z002HomeBaseHandler extends Handler {
...
export class Z002HomeHandler extends Z002HomeBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const z002homeConfig = this.domain.z002home_config;
        console.log('z002homeConfig', z002homeConfig);

        // 检查 processingConfig 是否为 undefined
        if (!z002homeConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 z002home' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('z002homeConfig', z002homeConfig);
        const info = yaml.load(z002homeConfig) as any;
        ...
        this.response.template = 'z002home_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('z002home_main', '/z002home', Z002HomeHandler, PERM.PERM_VIEW_Z002HOME);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('z002home')) {
            console.log('Z002Home Domain pass');
            return true;
        }
        console.log('Z002Home Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'z002home') {
                ...
                console.log('Z002Home SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('Z002Home SystemConfig pass');
                    return true;
                }else{
                    console.log('Z002Home SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'z002home_main', { prefix: 'z002home' }, CheckAll);

   ctx.on('handler/after/Home#get', async (that) => {
    
        if (that.domain._id === 'Z002') {
            that.response.redirect = that.url('z002home_main');
        }
