...
export class B001HomeBaseHandler extends Handler {
...
export class B001HomeHandler extends B001HomeBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const b001homeConfig = this.domain.b001home_config;
        console.log('b001homeConfig', b001homeConfig);

        // 检查 processingConfig 是否为 undefined
        if (!b001homeConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 b001home' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('b001homeConfig', b001homeConfig);
        const info = yaml.load(b001homeConfig) as any;
        ...
        this.response.template = 'b001home_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('b001home_main', '/b001home', B001HomeHandler, PERM.PERM_VIEW_B001Home);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('b001home')) {
            console.log('B001Home Domain pass');
            return true;
        }
        console.log('B001Home Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'b001home') {
                ...
                console.log('B001Home SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('B001Home SystemConfig pass');
                    return true;
                }else{
                    console.log('B001Home SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'b001home_main', { prefix: 'b001home' }, CheckAll);

   ctx.on('handler/after/Home#get', async (that) => {
    
        if (that.domain._id === 'B001') {
            that.response.redirect = that.url('b001home_main');
        }
