...
export class B001HubBaseHandler extends Handler {
...
export class B001HubHandler extends B001HubBaseHandler {
    uids = new Set<number>();
    ...
    async getHubwork(domainId: string, limit = 5) {
    ...
    async get({ domainId }) {
        const b001hubConfig = this.domain.b001hub_config;
        console.log('b001hubConfig', b001hubConfig);

        // 检查 processingConfig 是否为 undefined
        if (!b001hubConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 b001hub' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('b001hubConfig', b001hubConfig);
        const info = yaml.load(b001hubConfig) as any;
        ...
        this.response.template = 'b001hub_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('b001hub_main', '/b001hub', B001HubHandler, PERM.PERM_VIEW_B001HUB);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('b001hub')) {
            console.log('B001Hub Domain pass');
            return true;
        }
        console.log('B001Hub Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'b001hub') {
                ...
                console.log('B001Hub SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('B001Hub SystemConfig pass');
                    return true;
                }else{
                    console.log('B001Hub SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'b001hub_main', { prefix: 'b001hub' }, CheckAll);
