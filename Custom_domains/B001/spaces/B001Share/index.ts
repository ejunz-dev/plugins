...
export class B001ShareBaseHandler extends Handler {
...
export class B001ShareHandler extends B001ShareBaseHandler {
    uids = new Set<number>();
    ...
    async getSharework(domainId: string, limit = 5) {
    ...
    async get({ domainId }) {
        const b001shareConfig = this.domain.b001share_config;
        console.log('b001shareConfig', b001shareConfig);

        // 检查 processingConfig 是否为 undefined
        if (!b001shareConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 b001share' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('b001shareConfig', b001shareConfig);
        const info = yaml.load(b001shareConfig) as any;
        ...
        this.response.template = 'b001share_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    const CheckSpaceStore = (h) => {
        ...
        if (availableSpaces.has('b001share')) {
            console.log('B001Share Domain pass');
            return true;
        }
        console.log('B001Share Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'b001share') {
                ...
                console.log('B001Share SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('B001Share SystemConfig pass');
                    return true;
                }else{
                    console.log('B001Share SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'b001share_main', { prefix: 'b001share' }, CheckAll);
