...
export class A001TeamBaseHandler extends Handler {
...
export class A001TeamHandler extends A001TeamBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const a001teamConfig = this.domain.a001team_config;
        console.log('a001teamConfig', a001teamConfig);

        // 检查 processingConfig 是否为 undefined
        if (!a001teamConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 a001team' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('a001teamConfig', a001teamConfig);
        const info = yaml.load(a001teamConfig) as any;
        ...
        this.response.template = 'a001team_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    const CheckSpaceStore = (h) => {
        ...
        if (availableSpaces.has('a001team')) {
            console.log('A001Team Domain pass');
            return true;
        }
        console.log('A001Team Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'a001team') {
                ...
                console.log('A001Team SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('A001Team SystemConfig pass');
                    return true;
                }else{
                    console.log('A001Team SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'a001team_main', { prefix: 'a001team' }, CheckAll);
