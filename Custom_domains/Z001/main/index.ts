...
export async function apply(ctx: Context) {
    ...
    ctx.once('handler/before/DomainEdit#get', async (that) => {
        if (that.domain._id === 'Z001') 
            if (!that.domain.z001_announce1) {
            SettingModel.DomainSetting(
                SettingModel.Setting('setting_domain', 'z001_announce1', '', 'markdown', 'Z001 Announce1'),
            );
            if (!that.domain.z001_announce2) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z001_announce2', '', 'markdown', 'Z001 Announce2'),
                );
            }
            if (!that.domain.z001_announce3) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z001_announce3', '', 'markdown', 'Z001 Announce3'),
                );
            }
            if (!that.domain.z001_announce4) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z001_announce4', '', 'markdown', 'Z001 Announce4'),
                );
            }
            if (!that.domain.z001_announce5) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z001_announce5', '', 'markdown', 'Z001 Announce5'),
                );
            }
            if (!that.domain.z001_announce6) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'z001_announce6', '', 'markdown', 'Z001 Announce6'),
                );
            }       
         }
    });
