...
export class A003HomeBaseHandler extends Handler {
...
export class A003HomeHandler extends A003HomeBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const a003homeConfig = this.domain.a003home_config;
        console.log('a003homeConfig', a003homeConfig);

        // 检查 processingConfig 是否为 undefined
        if (!a003homeConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 a003home' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('a003homeConfig', a003homeConfig);
        const info = yaml.load(a003homeConfig) as any;
        ...
        this.response.template = 'a003home_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('a003home_main', '/a003home', A003HomeHandler, PERM.PERM_VIEW_A003HOME);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('a003home')) {
            console.log('A003Home Domain pass');
            return true;
        }
        console.log('A003Home Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'a003home') {
                ...
                console.log('A003Home SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('A003Home SystemConfig pass');
                    return true;
                }else{
                    console.log('A003Home SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'a003home_main', { prefix: 'a003home' }, CheckAll);

   ctx.on('handler/after/Home#get', async (that) => {
    
        if (that.domain._id === 'A003') {
            that.response.redirect = that.url('a003home_main');
        }
