...
export async function apply(ctx: Context) {
    ctx.i18n.load('en', {
        'a003home_main': 'Delta Force',
        'a003home': 'Delta Force',
    });
    ...
    ctx.once('handler/before/DomainEdit#get', async (that) => {
        if (that.domain._id === 'A003') 
            if (!that.domain.a003_announce1) {
            SettingModel.DomainSetting(
                SettingModel.Setting('setting_domain', 'a003_announce1', '', 'markdown', 'A003 Announce1'),
            );
            if (!that.domain.a003_announce2) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a003_announce2', '', 'markdown', 'A003 Announce2'),
                );
            }
            if (!that.domain.a003_announce3) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a003_announce3', '', 'markdown', 'A003 Announce3'),
                );
            }
            if (!that.domain.a003_announce4) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a003_announce4', '', 'markdown', 'A003 Announce4'),
                );
            }
            if (!that.domain.a003_announce5) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a003_announce5', '', 'markdown', 'A003 Announce5'),
                );
            }
            if (!that.domain.a003_announce6) {
                SettingModel.DomainSetting(
                    SettingModel.Setting('setting_domain', 'a003_announce6', '', 'markdown', 'A003 Announce6'),
                );
            }       
         }
    });
