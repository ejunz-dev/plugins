...
import {
    ...
    UserModel,ContestModel,ServiceModel,DiscussionModel,ProblemModel,camelCase, md5 
} from 'ejun';
...
export class EjunzServiceBaseHandler extends Handler {
...
export class EjunzServiceHandler extends EjunzServiceBaseHandler {
    uids = new Set<number>();
    ...
    async getService(domainId: string, limit = 10) {
        if (!this.user.hasPerm(PERM.PERM_VIEW_TRAINING)) return [[], {}];
        const tdocs = await ServiceModel.getMulti(domainId)
            .sort({ pin: -1, _id: 1 }).limit(limit).toArray();
        const tsdict = await ServiceModel.getListStatus(
            domainId, this.user._id, tdocs.map((tdoc) => tdoc.docId),
        );
        return [tdocs, tsdict];
    }
    ...
    async get({ domainId }) {
        const ejunzserviceConfig = this.domain.ejunzservice_config;
        console.log('ejunzserviceConfig', ejunzserviceConfig);

        // 检查 processingConfig 是否为 undefined
        if (!ejunzserviceConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 ejunzservice' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('ejunzserviceConfig', ejunzserviceConfig);
        const info = yaml.load(ejunzserviceConfig) as any;
        ...
        this.response.template = 'ejunzservice_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('ejunzservice_main', '/ejunzservice', EjunzServiceHandler, PERM.PERM_VIEW_EjunzTRAINING);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('ejunzservice')) {
            console.log('EjunzService Domain pass');
            return true;
        }
        console.log('EjunzService Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'ejunzservice') {
                const beforeSystemSpace = SystemModel.get(s.key);
                const parsedBeforeSystemSpace = yaml.load(beforeSystemSpace) as any[];
                console.log('EjunzService SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('EjunzService SystemConfig pass');
                    return true;
                }else{
                    console.log('EjunzService SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'ejunzservice_main', { prefix: 'ejunzservice' }, CheckAll);
