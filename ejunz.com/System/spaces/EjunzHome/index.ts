...
export class EjunzHomeBaseHandler extends Handler {
...
export class EjunzHomeHandler extends EjunzHomeBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const ejunzhomeConfig = this.domain.ejunzhome_config;
        console.log('ejunzhomeConfig', ejunzhomeConfig);

        // 检查 processingConfig 是否为 undefined
        if (!ejunzhomeConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 ejunzhome' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('ejunzhomeConfig', ejunzhomeConfig);
        const info = yaml.load(ejunzhomeConfig) as any;
        ...
        this.response.template = 'ejunzhome_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    const CheckSpaceStore = (h) => {
        ...
        if (availableSpaces.has('ejunzhome')) {
            console.log('EjunzHome Domain pass');
            return true;
        }
        console.log('EjunzHome Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'ejunzhome') {
                ...
                console.log('EjunzHome SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('EjunzHome SystemConfig pass');
                    return true;
                }else{
                    console.log('EjunzHome SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'ejunzhome_main', { prefix: 'ejunzhome' }, CheckAll);

   ctx.on('handler/after/Home#get', async (that) => {
    
        if (that.domain._id === 'system') {
            that.response.redirect = that.url('ejunzhome_main');
        }
