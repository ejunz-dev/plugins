...
export class EjunzCommunityBaseHandler extends Handler {
...
export class EjunzCommunityHandler extends EjunzCommunityBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const ejunzcommunityConfig = this.domain.ejunzcommunity_config;
        console.log('ejunzcommunityConfig', ejunzcommunityConfig);

        // 检查 processingConfig 是否为 undefined
        if (!ejunzcommunityConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 ejunzcommunity' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('ejunzcommunityConfig', ejunzcommunityConfig);
        const info = yaml.load(ejunzcommunityConfig) as any;
        ...
        this.response.template = 'ejunzcommunity_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    SettingModel.DomainSpaceConfigSetting(
        ...
        (   
            'spaces', 
            'ejunzcommunity_config', 
            [], 
            'yaml', 
            'ejunzcommunity_front'
        ),
    );
    SettingModel.DomainSpacePluginSetting(
        ...
        (   
            'spaces', 
            'ejunzcommunity_plugin', 
            [], 
            'yaml',
            'ejunzcommunity_plugins'
        ),
    );

    const PERM = {
        PERM_VIEW_EjunzCOMMUNITY: 1n << 85n,
    };

    global.Ejunz.model.builtin.registerPluginPermission(
        'spaces',
        PERM.PERM_VIEW_EjunzCOMMUNITY, 
        'View EjunzCommunity',
        false,
        true,
        'ejunzcommunity'
    ...
    ctx.Route('ejunzcommunity_main', '/ejunzcommunity', EjunzCommunityHandler, PERM.PERM_VIEW_EjunzCOMMUNITY);


    const CheckSpaceStore = (h) => {
        const availableSpaces = new Set(yaml.load(h.domain.spaces) as string[]);
        if (availableSpaces.has('ejunzcommunity')) {
            console.log('EjunzCommunity Domain pass');
            return true;
        }
        console.log('EjunzCommunity Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'ejunzcommunity') {
                const beforeSystemSpace = SystemModel.get(s.key);
                const parsedBeforeSystemSpace = yaml.load(beforeSystemSpace) as any[];
                console.log('EjunzCommunity SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('EjunzCommunity SystemConfig pass');
                    return true;
                }else{
                    console.log('EjunzCommunity SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'ejunzcommunity_main', { prefix: 'ejunzcommunity' }, CheckAll);
