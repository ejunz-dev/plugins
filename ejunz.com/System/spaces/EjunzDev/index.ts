...
export class EjunzDevBaseHandler extends Handler {
...
export class EjunzDevHandler extends EjunzDevBaseHandler {
    uids = new Set<number>();
    ...
    async get({ domainId }) {
        const ejunzdevConfig = this.domain.ejunzdev_config;
        console.log('ejunzdevConfig', ejunzdevConfig);

        // 检查 processingConfig 是否为 undefined
        if (!ejunzdevConfig) {
            this.response.body = {
                contents: [{ message: '需要进行配置 ejunzdev' }],
                udict: {},
                domain: this.domain,
            };
            return;
        ...
        console.log('ejunzdevConfig', ejunzdevConfig);
        const info = yaml.load(ejunzdevConfig) as any;
        ...
        this.response.template = 'ejunzdev_main.html';
        this.response.body = {
            contents,
            udict,
            domain: this.domain,
...
export async function apply(ctx: Context) {
    ...
    const PERM = {
        PERM_VIEW_EjunzDEV: 1n << 86n,
    };

    global.Ejunz.model.builtin.registerPluginPermission(
        'spaces',
        PERM.PERM_VIEW_EjunzDEV, 
        'View EjunzDev',
        false,
        true,
        'ejunzdev'
    );
    ctx.Route('ejunzdev_main', '/ejunzdev', EjunzDevHandler, PERM.PERM_VIEW_EjunzDEV);
    ...
    const CheckSpaceStore = (h) => {
        ...
        if (availableSpaces.has('ejunzdev')) {
            console.log('EjunzDev Domain pass');
            return true;
        }
        console.log('EjunzDev Domain fail');
        return false;
    }
    ...
    const CheckSystemConfig = (h) => {
        ...
        for (const s of systemspaces) {
            if (s.name == 'ejunzdev') {
                ...
                console.log('EjunzDev SystemConfig', parsedBeforeSystemSpace);
                if (parsedBeforeSystemSpace.includes(h.domain._id)) {
                    console.log('EjunzDev SystemConfig pass');
                    return true;
                }else{
                    console.log('EjunzDev SystemConfig fail');
                    return false;
                }
            }
        }
   ...
   ctx.injectUI('NavMainDropdown', 'ejunzdev_main', { prefix: 'ejunzdev' }, CheckAll);
